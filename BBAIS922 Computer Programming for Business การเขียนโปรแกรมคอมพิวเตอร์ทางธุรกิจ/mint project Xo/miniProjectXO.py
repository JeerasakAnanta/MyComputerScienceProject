# Dev by MR.Jeerasak Ananta
# Computer science RMTUL NAN

# import  tkinter for window UI app
from tkinter import *
from tkinter import ttk
import tkinter.messagebox

application = Tk()
application.title("เกมส์  Tic Tac Toe (XO)")

# add Buttons สร้างปุ่มการทำงาน  3 X 3
# but1 to but 9 เพิ่มคลิกของการทำงาน
bu1 = ttk.Button(application, text=' ')
bu1.grid(row=0, column=0, sticky='snew', ipadx=40, ipady=40)
bu1.config(command=lambda: ButtonClick(1))

bu2 = ttk.Button(application, text=' ')
bu2.grid(row=0, column=1, sticky='snew', ipadx=40, ipady=40)
bu2.config(command=lambda: ButtonClick(2))

bu3 = ttk.Button(application, text=' ')
bu3.grid(row=0, column=2, sticky='snew', ipadx=40, ipady=40)
bu3.config(command=lambda: ButtonClick(3))

bu4 = ttk.Button(application, text=' ')
bu4.grid(row=1, column=0, sticky='snew', ipadx=40, ipady=40)
bu4.config(command=lambda: ButtonClick(4))

bu5 = ttk.Button(application, text=' ')
bu5.grid(row=1, column=1, sticky='snew', ipadx=40, ipady=40)
bu5.config(command=lambda: ButtonClick(5))

bu6 = ttk.Button(application, text=' ')
bu6.grid(row=1, column=2, sticky='snew', ipadx=40, ipady=40)
bu6.config(command=lambda: ButtonClick(6))

bu7 = ttk.Button(application, text=' ')
bu7.grid(row=2, column=0, sticky='snew', ipadx=40, ipady=40)
bu7.config(command=lambda: ButtonClick(7))

bu8 = ttk.Button(application, text=' ')
bu8.grid(row=2, column=1, sticky='snew', ipadx=40, ipady=40)
bu8.config(command=lambda: ButtonClick(8))

bu9 = ttk.Button(application, text=' ')
bu9.grid(row=2, column=2, sticky='snew', ipadx=40, ipady=40)
bu9.config(command=lambda: ButtonClick(9))


# เเสดงผลให้เห็นการ ผู้เล่นที่ 1 ทำการเริ่มขึ้นมา ก่อน
playerturn = ttk.Label(application, text="  ผู้เล่นที่หนึ่ง(1) เริ่ม  ")
playerturn.grid(row=3, column=0, sticky='snew', ipadx=40, ipady=40)

# กำหนดให้ ผู้เล่น  1 เป็น X และ ผู้เล่นที่  2 ให้ป็น  o
playerdetails = ttk.Label(
    application, text="    ผู้เล่นที่หนึ่ง(1) ให้เป็น X\n\n    ผู้เล่นที่สอง(2) ให้เป็น O")
playerdetails.grid(row=3, column=2, sticky='snew', ipadx=40, ipady=40)

Restart = ttk.Button(application, text='เริ่มต้นใหม่(Restart)')
Restart.grid(row=3, column=1, sticky='snew', ipadx=40, ipady=40)
Restart.config(command=lambda: restartbutton())

#  cardite for me  
cardit = ttk.Button(application, text="hello")
cardit.grid(row=3, column=2, ipadx=5,  ipady=5)

a = 1
b = 0
c = 0

# ฟังก์ชั้นทำหน้าที ให้ปุ่มกดเป็นค่าที่เริ่มต้น
...
def restartbutton():
    ...
    b = 0
    c = 0
    playerturn['text'] = "   ผู้เล่นที่หนึ่ง(1) เริ่ม!!   "
    bu1['text'] = ' '
    bu2['text'] = ' '
    bu3['text'] = ' '
    bu4['text'] = ' '
...
#  ฟังก์ชั้นทำหน้าที่ กดปุ่มทำงาน
...
def ButtonClick(id):
    ...
    #  ตัวสอบถ้าเป็นไปตามกฏ จะ
    # เเนวตัง หรือ เเนวทะเเยง หรือ 
    # winnder 1 winder
    #
    if (bu1['text'] == 'X' and bu2['text'] == 'X' and bu3['text'] == 'X' or
        ...
        disableButton()
        c = 1
        tkinter.messagebox.showinfo("สรุปผลการเล่น", "ผู้เล่นที่หนึ่ง ชนะ :)")
        ...
        disableButton()
        c = 1
        tkinter.messagebox.showinfo("สรุปผลของการเล่น", "ผู้เล่นที่สอง ชนะ :)")
        
    # เมื่อทำงาน กดปุ่มหนดเเล้ว หรือ ครบจำนวนทั้งหมดเท่ากับ  9
    elif b == 9:
        disableButton()
        c = 1
        tkinter.messagebox.showinfo("สรุปผลของการเล่น", "เสมอ. :P")

    # ให้ผู้เล่นเริ่มที่
    if a == 1 and c == 0:
        playerturn['text'] = "   ผู้เล่นคนที่ 1 turn!   "
    elif a == 0 and c == 0:
        playerturn['text'] = "   ผู้เล่นคนที่  2 turn!   "


# loop การทำงานของเกมส์
